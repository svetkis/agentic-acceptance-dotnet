...
| **IntegrationDatabaseTest** | EF Core InMemory is not a database: integration tests run against real Postgres in Docker (Testcontainers); catches `AsNoTracking` leaking into the write path, lost transactions, broken SQL translation — everything InMemory silently forgives | [tests/patterns/IntegrationDatabaseTest.cs](../tests/patterns/IntegrationDatabaseTest.cs) | — |
| **CharacterizationTest** | Golden master for spec-less code: refactor must reproduce recorded behavior; boundary-first deterministic inputs, pinned environment | [tests/patterns/CharacterizationTest.cs](../tests/patterns/CharacterizationTest.cs) | `examples/DemoProject/tests/DemoProject.Tests/AvailableSlotsCharacterizationTests.cs` + `GoldenMasters/available-slots.json` + `LegacySlotSearch.cs`; Example B (Monte Carlo tolerance): `SmartSelectionCharacterizationTests.cs` + `ScheduleSimulator.cs`; red demo in `DemoProject.Traps.Tests/CharacterizationTrapTests.cs` |
...
| **BenchmarkTest** | Evidence for hot-path optimizations: BenchmarkDotNet A/B run with `[MemoryDiagnoser]`, ratio-based CI gate, never absolute numbers | [tests/patterns/BenchmarkTest.cs](../tests/patterns/BenchmarkTest.cs) | — |
...
|----------|---------------|
| [Obstacles: context-rot.md](obstacles/context-rot.md) | Inherent LLM limitation ("context rot") that cannot be eliminated — only compensated; why guardrails must stay stateless |
...
| [ai-patterns.md](solutions/ai-patterns.md) | 10 AI-driven development patterns (observed cases) |
| [human-audit-bridge.md](solutions/human-audit-bridge.md) | How to use AI checklists for manual human audit |
| [e2e-mcp-testing.md](solutions/e2e-mcp-testing.md) | Agent-driven E2E via MCP on a deployed stand: evidence-first scenario reports, what to cover and what it is not |
...
| [nuget-audit-as-error.md](solutions/nuget-audit-as-error.md) | NuGet audit warnings (NU1901–NU1904: vulnerable packages low→critical, direct and transitive) as build errors — Level 1 gate that hardens version-audit |
...
| [ci/scripts/verify-before-push.sh](../ci/scripts/verify-before-push.sh) | Local CI-parity check before pushing: repo scripts + Release build with warnings-as-errors + demo suites (green and expected-red) |
