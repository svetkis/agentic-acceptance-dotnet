...
| **Verify (Verify.TUnit)** | Snapshot testing library: `await Verify(obj)` compares against a `.verified` file; first run produces a `.received` file that must be consciously accepted. Auto-scrubs `Guid`/`DateTime`. Traps: blind snapshot acceptance, TUnit version lockstep, Verify 33+ SponsorCheck license gate (SC021). | [tests/patterns/VerifySnapshotTest.cs](tests/patterns/VerifySnapshotTest.cs) |
| **Storm Petrel** | Snapshot testing via source generator: baselines live in test code; a generated `*TestStormPetrel` copy rewrites them on demand — contract changes show up as code diffs, not accepted files. MIT, no license gates. xUnit/NUnit/MSTest out of the box; TUnit needs env-var attribute config + Reflection mode. | [tests/patterns/StormPetrelSnapshotTest.cs](tests/patterns/StormPetrelSnapshotTest.cs) |
| **Characterization test** | A test that captures current system behavior without judging correctness (golden master: deterministic inputs in, committed outputs out). Needed so refactoring spec-less code does not change behavior. | [tests/patterns/CharacterizationTest.cs](tests/patterns/CharacterizationTest.cs) |
...
| **Testcontainers** | Infrastructure for running real databases (PostgreSQL, Redis) in Docker containers during tests. Alternative to EF Core InMemory provider. | [tests/patterns/IntegrationDatabaseTest.cs](tests/patterns/IntegrationDatabaseTest.cs), [docs/traps/testing.md#silent-breakdown](docs/traps/testing.md#silent-breakdown) |
