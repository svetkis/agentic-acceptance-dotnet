...
- [ ] `bash ci/scripts/verify-before-push.sh` passes (Release build with warnings-as-errors + repo checks + demo suites — the same things CI runs; a local Debug-only build is NOT enough)
